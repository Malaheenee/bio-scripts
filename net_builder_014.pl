...
($detal, $data_file, $output_filename, $size, $transparent, $nocancer, $nolegend) = undef;
GetOptions(
...
open(DATA_FILE, "< $data_file") or die "\n    Can't open file $data_file: $!";
  while($string = <DATA_FILE>) {
    ...
    @genes = split(/,/, $value3);
    if ($#forms == 0) {
      $Genes{$key}->{Coord} = [0, 0, 2, $forms[0]+2, $forms[0]];}
    else {
...
sub MakeSubNet {
  ...
  if ($elnum != 0) {
    ...
    foreach $new1 (keys %Genes) {
      @keys2 = keys %{$Genes{$new1}};
      if ($#keys2 == 0) {
        $x = ($d*2*$pi*$elnum{X})*cos($a{X}) + $centerx;
        $y = ($d*2*$pi*$elnum{X})*sin($a{X}) + $centery;
        $a{X} += (2*$pi/$elnum{X});
      }
      else {
        foreach (0..$detal-1) {
        if ($Genes{$new1}->{Coord}->[$d] == $_) {
          $x = (($d+2)*$pi*$elnum{$_})*cos($a{$_}) + $Centers{$_}->[0];
          $y = (($d+2)*$pi*$elnum{$_})*sin($a{$_}) + $Centers{$_}->[1];
          $a{$_} += (2*$pi/$elnum{$_});
          }
        }
      }
    ...
    foreach $new1 (keys %Genes) {
      if ($Genes{$new1}->{Coord}->[4] == 0) {
        ...
        # Ромбы
        $poly = new GD::Polygon;
        $poly->addPt($Genes{$new1}->{Coord}->[0], $Genes{$new1}->{Coord}->[1] - $half_length/1.5);
        $poly->addPt($Genes{$new1}->{Coord}->[0] + $half_length/1.5, $Genes{$new1}->{Coord}->[1]);
        $poly->addPt($Genes{$new1}->{Coord}->[0], $Genes{$new1}->{Coord}->[1] + $half_length/1.5);
        $poly->addPt($Genes{$new1}->{Coord}->[0] - $half_length/1.5, $Genes{$new1}->{Coord}->[1]);
        $im->filledPolygon($poly, $Genes{$new1}->{Coord}->[3]);
      }
        ...
        $poly->addPt($Genes{$new1}->{Coord}->[0],                  $Genes{$new1}->{Coord}->[1] - $half_length/1.5);
        $poly->addPt($Genes{$new1}->{Coord}->[0] + $half_length/1.5, $Genes{$new1}->{Coord}->[1]);
        $poly->addPt($Genes{$new1}->{Coord}->[0] + $half_length/3, $Genes{$new1}->{Coord}->[1] + $half_length/1.5);
        $poly->addPt($Genes{$new1}->{Coord}->[0] - $half_length/3, $Genes{$new1}->{Coord}->[1] + $half_length/1.5);
        $poly->addPt($Genes{$new1}->{Coord}->[0] - $half_length/1.5, $Genes{$new1}->{Coord}->[1]);
        $im->filledPolygon($poly, $Genes{$new1}->{Coord}->[3]);
      }
        ...
        # Шестиугольник
        $poly = new GD::Polygon;
        $poly->addPt($Genes{$new1}->{Coord}->[0],                  $Genes{$new1}->{Coord}->[1] - $half_length);
        $poly->addPt($Genes{$new1}->{Coord}->[0] + $half_length/2, $Genes{$new1}->{Coord}->[1] - $half_length/2);
        $poly->addPt($Genes{$new1}->{Coord}->[0] + $half_length/2, $Genes{$new1}->{Coord}->[1] + $half_length/2);
        $poly->addPt($Genes{$new1}->{Coord}->[0],                  $Genes{$new1}->{Coord}->[1] + $half_length);
        $poly->addPt($Genes{$new1}->{Coord}->[0] - $half_length/2, $Genes{$new1}->{Coord}->[1] + $half_length/2);
...
sub Usage_Version ($) {
    if ($_[0] eq "version") {
      ...
      print "-l, --nolegend \t\t\t (defaults no)\n";
#      print "-c, --nocolor \t\t\t (defaults no)\n";
#      print "-f, --noforms \t\t\t (defaults no)\n";
